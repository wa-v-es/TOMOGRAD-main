...
REAL, DIMENSION(maxg,maxg) :: gradientmax,gradientmin,gradient,gradientmax_az,gradientmin_az,gradient_az,ave_std
...
!     WRITE(11,*)gridlattmp,gridlontmp,gradient(j,k),gradient_az(j,k),ave_std(j,k)

     !Print out gradient at all azimuths (1:num_az)
     DO m=1,num_az
!     WRITE(11,form)gridlattmp,gridlontmp,azgradient(m),az(m),std(m)

     IF(az(m)>180)az(m)=(az(m)-360)
     WRITE(11,form)gridlattmp,gridlontmp,azgradient(m),az(m)
     END DO ! m loop
